...
export default function Navbar() {
  ...
  return (
    <nav className="w-full absolute top-0 md:top-5 lg:top-0 left-0 z-50 bg-transparent">
      ...
      ${menuOpen ? "rotate-45 top-1/2 bg-white" : "top-2 bg-black shadow-lg"}`}
          />

          {/* Middle Line */}
          <span
            className={`absolute w-8 md:w-12 h-0.5 transition-opacity duration-300 origin-center 
      ${menuOpen ? "opacity-0 bg-white" : "top-1/2 bg-black shadow-lg"}`}
          />

          {/* Bottom Line */}
          <span
            className={`absolute w-8 md:w-12 h-0.5 transition-transform duration-300 origin-center 
      ${menuOpen ? "-rotate-45 top-1/2 bg-white" : "top-[2rem] bg-black shadow-lg"}`}
          />
        </button>
        ...
        <div className="flex justify-end z-50">
          <a
            href="/#contact"
            className={`px-4 text-xs md:text-base py-2 rounded  transition ${menuOpen ? "backdrop-blur-lg bg-white text-[#9B177E] hover:bg-[#9B177E] hover:text-black rounded-3xl" : "bg-[#9B177E] text-white hover:text-black"}`}
          >
            Contact me
          </a>
        </div>
      ...
      <AnimatePresence>
        {menuOpen && (
          ...
          >
            ...
            <Link href="/#equipment" onClick={toggleMenu} className="hover:text-[black] anton-font">{'Equipment'}</Link>
